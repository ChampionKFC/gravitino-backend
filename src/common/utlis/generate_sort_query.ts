...
export function generateWhereQuery(filters) {
  ...
  for (const key of filterKeyValues.keys()) {
    ...
    const isDateTime = moment(value, `'yyyy-MM-DDTHH:mm:ss.SSSZ'`, true).isValid()

    if (Array.isArray(value)) {
      let whereSubQuery = ``
      for (let index = 0; index < value.length; index++) {
        const element = value[index]
        if ((typeof element == 'string' || element instanceof String) && !isDateTime) {
      ...
      if (whereSubQuery !== '') {
        whereQuery += `(${whereSubQuery.substring(0, whereSubQuery.length - 4)})`
      }
    } else {
      if ((typeof value == 'string' || value instanceof String) && !isDateTime) {
...
function getObjectKeyValues(obj, isWhereQuery = true, previousPath = '', objectKeys = [], keyValues = new Map<string, any>()) {
  Object.keys(obj).forEach((key) => {
    const currentPath = previousPath ? `${previousPath}.${key}` : key

    let value = obj[key]
    if (typeof value !== 'object') {
      if ((typeof value == 'string' || value instanceof String) && isWhereQuery) {
        value = `'${value}'`
      }
      ...
      getObjectKeyValues(value, isWhereQuery, currentPath, objectKeys, keyValues)
    }
  })
