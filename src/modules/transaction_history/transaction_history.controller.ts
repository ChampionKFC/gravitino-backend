...
export class TransactionHistoryController {
  ...
  async create(
    ...
    if (createTransactionHistoryDto.user_id) {
      const foundUser = await this.userService.findOne(
        createTransactionHistoryDto.user_id,
      );
      if (!foundUser) {
  ...
  async update(
    ...
    if (updateTransactionHistoryDto.history_id) {
      foundTransactionHistory = await this.transactionHistoryService.findOne(
        updateTransactionHistoryDto.history_id,
      );
    }
    if (!foundTransactionHistory) {
      throw new HttpException(
        AppError.TRANSACTION_HISTORY_NOT_FOUND,
    ...
    if (updateTransactionHistoryDto.user_id) {
      const foundUser = await this.userService.findOne(
        updateTransactionHistoryDto.user_id,
      );
      if (!foundUser) {
        throw new HttpException(AppError.USER_NOT_FOUND, HttpStatus.NOT_FOUND);
      }
  ...
  async remove(@Param('id') id: number) {
    const foundTransactionHistory =
      await this.transactionHistoryService.findOne(id);
